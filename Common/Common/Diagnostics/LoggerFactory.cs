...
namespace CodeStack.SwEx.Common.Diagnostics {
    ...
    public static class LoggerFactory {
        ...
        public static ILogger Create(Type moduleType, string subModuleName = "") {
            ...
            if(moduleType.TryGetAttribute<LoggerOptionsAttribute>(out var log)) {
                logCallStack = log.LogCallStack;
                loggerType = log.Type;
                logName = log.Name;
            }

            if(!moduleType.TryGetAttribute<ModuleInfoAttribute>(out var moduleName)) 
