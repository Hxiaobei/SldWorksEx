...
namespace SolidWorks.Interop.sldworks {
    ...
    public static class ModelerEx {
        private static readonly IMathUtility m_MathUtils = Context.CurrentApp.IGetMathUtility();
        ...
        public static IBody2 CreateBox(this IModeler modeler, Point center, Vector dir, ref Vector refDir,
            ...
            var surf = CreatePlanarSurface(modeler, center, dir, ref refDir, out IMathVector refVec);

            var xVec = new Vector(refVec.ArrayData as double[]);
            var yVec = xVec.Cross(dir);

            Point GetPoint(double x, double y) => center.Move(xVec, x).Move(yVec, y);
            ...
            ICurve CreateCurve(Point p1, Point p2) => ((ICurve)modeler.CreateLine(p1.ToArray(), (p1 - p2).ToArray()))
            ...
            var zVec = (MathVector)m_MathUtils.CreateVector(dir.ToArray());
