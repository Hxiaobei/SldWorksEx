...
namespace CodeStack.SwEx.MacroFeature.Helpers {
    internal class MacroFeatureParametersParser {
        ...
        private readonly string[] m_CurrentIcons;
        ...
        internal static void ReleaseDisplayDimensions(IDisplayDimension[] dispDims) {
            if(dispDims == null) return;

            for(int i = 0; i < dispDims.Length; i++)
                if(dispDims[i] != null && Marshal.IsComObject(dispDims[i]))
                    Marshal.ReleaseComObject(dispDims[i]);
        ...
        internal TParams GetParameters<TParams>(IFeature feat, IMacroFeatureData featData, IModelDoc2 model,
                    out IDisplayDimension[] dispDims, out string[] dispDimParams, out IBody2[] editBodies, out OutdateState_e state)
        ...
        internal object GetParameters(IFeature feat, IMacroFeatureData featData, IModelDoc2 model, Type paramsType,
            ...
            featData.GetSelections3(out var retSelObj, out var selObjType, out var selMarks, out var selDrViews, out var compXforms);
            ...
            try {
                ...
                if(retParamNames is string[] paramNames
                    && retParamValues is string[] paramValues) {
        ...
        internal void Parse(object parameters,
            ...
            if(parameters.GetType().TryGetAttribute<ParametersVersionAttribute>(out var par)) {
                ...
                setVersionFunc.Invoke(VERSION_PARAMETERS_NAME, par.Version);
                setVersionFunc.Invoke(VERSION_DIMENSIONS_NAME, par.Version);
            }
        ...
        internal void SetParameters(IModelDoc2 model, IFeature feat,
            IMacroFeatureData featData, object parameters, out OutdateState_e state) {
            string[] paramNames;
            int[] paramTypes;
            string[] paramValues;
            object[] selection;
            ...
            if(paramNames.Any()) {
                ...
                if(state.HasFlag(OutdateState_e.Dimensions)) {
        ...
        private void ConvertParameters(Type paramsType, Version paramVersion,
            ...
            if(paramsType.TryGetAttribute<ParametersVersionAttribute>(out var par)) {
                versConv = par.VersionConverter;
                curParamVersion = par.Version;
            }
        ...
        private OutdateState_e GetState(IMacroFeatureData featData, IDisplayDimension[] dispDims) {
            var state = OutdateState_e.UpToDate;

            if(dispDims != null && dispDims.Any(d => d is DisplayDimensionPlaceholder)) {
                state |= OutdateState_e.Dimensions;
            }
        ...
        private void SetAndReleaseDimension(IDisplayDimension dispDim, int index, double val, string confName) {
            ...
            dim.SetSystemValue3(val, (int)swSetValueInConfiguration_e.swSetValue_InSpecificConfigurations, new[] { confName });
